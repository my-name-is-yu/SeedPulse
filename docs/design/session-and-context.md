...
ゴール登録時に、Motivaが既存ゴール群と新規ゴールの関係をLLMに分析させる。
...
§7の「マルチゴールのコンテキスト分離」の原則は維持される。依存関係の管理は、セッションレベルのコンテキスト混入ではなく、Motivaのスケジューリングレイヤーで行われる。セッションは依然としてステートレスで独立だ。依存関係が影響するのは「いつ、どのゴールのループを回すか」の判断であり、「セッション内に何を含めるか」ではない。
