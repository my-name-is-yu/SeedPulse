...
> Conatus runs for years. Data accumulates without bound, but LLM context windows are finite.
...
Under long-term operation, Conatus continuously accumulates the following data:
...
"Delete the oldest first" is the simplest memory management strategy. But for Conatus, it does not work.
...
Long-term Memory
  ...
  │ Lifetime: as long as the Conatus instance exists
...
| Layer | What is retained | Format |
|-------|----------------|--------|
| Working | Not included (reports are Conatus→user output, not loop input) | |
...
Conatus already has a mechanism for judging "what matters" — DriveScorer (dissatisfaction, deadline, opportunity) and SatisficingJudge (whether something is good enough). Rather than a generic LRU, this existing motivation system is applied to memory management.
...
Dimensions with large gaps are areas Conatus is actively focused on. Short-term data related to these dimensions is retained beyond its normal retention period without compression.
...
When SatisficingJudge (`satisficing.md`) judges "this dimension is sufficient," the Short-term data for that dimension becomes a candidate for early Long-term compression, without waiting for the retention period. There is no value in retaining raw data for dimensions that Conatus no longer needs to focus on.
...
1. Create `~/.conatus/memory/` directory structure
...
| Motivation controls memory | Dynamic compression priority is set by DriveScorer/SatisficingJudge, not generic LRU |
