...
Conatus is the "control room for agents." Like a cockpit, a large amount of information is arranged in organized positions, and attention is drawn only when something is anomalous.
...
Route Handlers directly import conatus-core modules within the Node.js process (same process, no RPC needed).
...
Provider settings (`~/.conatus/provider.json`), plugin management (enable/disable), system health (CoreLoop state, EventServer connection count)
...
- [ ] TUI and Web UI can operate concurrently against the same Conatus process
