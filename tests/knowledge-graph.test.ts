...
describe("addNode", () => {
  it("adds a node and increases nodeCount", () => {
    expect(graph.nodeCount).toBe(0);
    graph.addNode("entry-1", "goal-A", ["tag1", "tag2"]);
    expect(graph.nodeCount).toBe(1);
  });

  it("stores correct node fields", () => {
    graph.addNode("entry-1", "goal-A", ["tag1", "tag2"]);
  ...
  it("duplicate ID updates existing node (upsert semantics)", () => {
    graph.addNode("entry-1", "goal-A", ["tag1"]);
    graph.addNode("entry-1", "goal-B", ["tag2", "tag3"]);
    expect(graph.nodeCount).toBe(1);
    const node = graph.getNode("entry-1");
  ...
  it("getAllNodes returns all added nodes", () => {
    graph.addNode("e1", "goal-A", []);
    graph.addNode("e2", "goal-A", []);
    graph.addNode("e3", "goal-B", []);
    expect(graph.getAllNodes()).toHaveLength(3);
  });
});

describe("removeNode", () => {
  it("removes the node and decreases nodeCount", () => {
    graph.addNode("e1", "goal-A", []);
    graph.addNode("e2", "goal-A", []);
    graph.removeNode("e1");
    expect(graph.nodeCount).toBe(1);
  ...
  it("removes associated edges when node is removed", () => {
    graph.addNode("e1", "goal-A", []);
    graph.addNode("e2", "goal-A", []);
    graph.addNode("e3", "goal-A", []);
    graph.addEdge({ from_id: "e1", to_id: "e2", relation: "supports", confidence: 0.8 });
    graph.addEdge({ from_id: "e3", to_id: "e1", relation: "refines", confidence: 0.7 });
    graph.addEdge({ from_id: "e2", to_id: "e3", relation: "depends_on", confidence: 0.9 });
  ...
  it("does nothing when removing a non-existent node", () => {
    graph.addNode("e1", "goal-A", []);
    graph.removeNode("does-not-exist"); // should not throw
    expect(graph.nodeCount).toBe(1);
  });
});
...
describe("addEdge", () => {
  it("creates an edge and increases edgeCount", () => {
    graph.addNode("e1", "goal-A", []);
    graph.addNode("e2", "goal-A", []);
    expect(graph.edgeCount).toBe(0);
    graph.addEdge({ from_id: "e1", to_id: "e2", relation: "supports", confidence: 0.9 });
  ...
  it("replacing duplicate edge (same from/to/relation) keeps count stable", () => {
    graph.addNode("e1", "goal-A", []);
    graph.addNode("e2", "goal-A", []);
    graph.addEdge({ from_id: "e1", to_id: "e2", relation: "supports", confidence: 0.5 });
    graph.addEdge({ from_id: "e1", to_id: "e2", relation: "supports", confidence: 0.9 });
    expect(graph.edgeCount).toBe(1);
...
describe("removeEdge", () => {
  it("removes a specific edge by from/to pair", () => {
    graph.addNode("e1", "goal-A", []);
    graph.addNode("e2", "goal-A", []);
    graph.addNode("e3", "goal-A", []);
    graph.addEdge({ from_id: "e1", to_id: "e2", relation: "supports", confidence: 0.8 });
    graph.addEdge({ from_id: "e1", to_id: "e3", relation: "contradicts", confidence: 0.6 });
  ...
  it("removes all edges between two nodes regardless of relation", () => {
    graph.addNode("e1", "goal-A", []);
    graph.addNode("e2", "goal-A", []);
    graph.addEdge({ from_id: "e1", to_id: "e2", relation: "supports", confidence: 0.8 });
    graph.addEdge({ from_id: "e1", to_id: "e2", relation: "refines", confidence: 0.7 });
  ...
  it("does nothing when edge does not exist", () => {
    graph.addNode("e1", "goal-A", []);
    graph.addNode("e2", "goal-A", []);
    graph.removeEdge("e1", "e2"); // should not throw
    expect(graph.edgeCount).toBe(0);
  });
});
...
describe("getEdgesFrom / getEdgesTo", () => {
  beforeEach(() => {
    ...
    graph.addNode("e3", "goal-A", []);
    graph.addEdge({ from_id: "e1", to_id: "e2", relation: "supports", confidence: 0.8 });
    graph.addEdge({ from_id: "e1", to_id: "e3", relation: "refines", confidence: 0.7 });
    graph.addEdge({ from_id: "e3", to_id: "e2", relation: "depends_on", confidence: 0.6 });
  });
...
describe("getRelated", () => {
  it("returns related nodes with their edges", () => {
    graph.addNode("e1", "goal-A", ["tag1"]);
    graph.addNode("e2", "goal-A", ["tag2"]);
    graph.addNode("e3", "goal-A", ["tag3"]);
    graph.addEdge({ from_id: "e1", to_id: "e2", relation: "supports", confidence: 0.9 });
    graph.addEdge({ from_id: "e1", to_id: "e3", relation: "contradicts", confidence: 0.5 });
  ...
  it("skips edges pointing to nodes not in the graph", () => {
    // Edge points to a non-existent node
    graph.addNode("e1", "goal-A", []);
    // Directly push an edge with a missing target (bypassing API)
    graph.addEdge({ from_id: "e1", to_id: "missing-node", relation: "supports", confidence: 0.8 });
    const related = graph.getRelated("e1");
...
describe("getContradictions", () => {
  it("returns only edges with relation=contradicts", () => {
    graph.addNode("e1", "goal-A", []);
    graph.addNode("e2", "goal-A", []);
    graph.addNode("e3", "goal-A", []);
    graph.addEdge({ from_id: "e1", to_id: "e2", relation: "supports", confidence: 0.9 });
    graph.addEdge({ from_id: "e2", to_id: "e3", relation: "contradicts", confidence: 0.7 });
    graph.addEdge({ from_id: "e1", to_id: "e3", relation: "contradicts", confidence: 0.6 });
...
describe("detectCycles", () => {
  it("detects a cycle in A→B→C→A", () => {
    graph.addNode("A", "goal-A", []);
    graph.addNode("B", "goal-A", []);
    graph.addNode("C", "goal-A", []);
    graph.addEdge({ from_id: "A", to_id: "B", relation: "depends_on", confidence: 1.0 });
    graph.addEdge({ from_id: "B", to_id: "C", relation: "depends_on", confidence: 1.0 });
    graph.addEdge({ from_id: "C", to_id: "A", relation: "depends_on", confidence: 1.0 });
  ...
  it("returns no cycles for a simple DAG", () => {
    graph.addNode("A", "goal-A", []);
    graph.addNode("B", "goal-A", []);
    graph.addNode("C", "goal-A", []);
    graph.addEdge({ from_id: "A", to_id: "B", relation: "supports", confidence: 0.9 });
    graph.addEdge({ from_id: "B", to_id: "C", relation: "supports", confidence: 0.9 });
  ...
  it("self-loop (A→A) is detected as a cycle", () => {
    graph.addNode("A", "goal-A", []);
    graph.addEdge({ from_id: "A", to_id: "A", relation: "depends_on", confidence: 0.5 });
...
describe("Persistence", () => {
  it("save/load round-trip preserves nodes and edges", () => {
    graph.addNode("e1", "goal-A", ["tag1", "tag2"]);
    graph.addNode("e2", "goal-B", ["tag3"]);
    graph.addEdge({ from_id: "e1", to_id: "e2", relation: "refines", confidence: 0.85 });
...
describe("clear", () => {
  ...
  it("persists empty state after clear", () => {
    graph.addNode("e1", "goal-A", []);
    graph.clear();
