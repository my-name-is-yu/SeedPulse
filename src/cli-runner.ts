...
Options:
  ...
  private extractCandidatePaths(targetPath: string, context: string, repoFiles: string[] = []): string[] {
  ...
  private ensureActionableSuggestion(suggestion: GoalSuggestion, targetPath: string, context: string, repoFiles: string[] = []): GoalSuggestion {
    const candidates = this.extractCandidatePaths(targetPath, context, repoFiles);
  ...
  private synthesizeFallbackSuggestions(targetPath: string, context: string, maxSuggestions: number, repoFiles: string[] = []): GoalSuggestion[] {
    const candidatePaths = this.extractCandidatePaths(targetPath, context, repoFiles);
    ...
    if (hasDocs || hasDesign || hasReadme) {
      addSuggestion(
  ...
  private normalizeSuggestPayload(
    rawOutput: unknown,
    targetPath: string,
    displayPath: string,
    context: string,
    ...
    const legacyNormalized = this.normalizeLegacySuggestPayload(
      rawOutput,
      targetPath,
      displayPath,
      context,
    ...
    const fallback = this.buildFallbackSuggestPayload(targetPath, displayPath, context, maxSuggestions, repoFiles);
  ...
  private normalizeLegacySuggestPayload(
    rawOutput: unknown,
    targetPath: string,
    displayPath: string,
    context: string,
    ...
    const candidateSource = this.extractSuggestCandidates(rawOutput);
    const normalizedSuggestions = candidateSource
      .map((candidate) => this.normalizeLegacySuggestion(candidate, targetPath, displayPath, context, repoFiles))
    ...
    return this.buildFallbackSuggestPayload(targetPath, displayPath, context, maxSuggestions, repoFiles);
  }

  private extractSuggestCandidates(rawOutput: unknown): unknown[] {
  ...
  private normalizeLegacySuggestion(
    candidate: unknown,
    targetPath: string,
    displayPath: string,
    context: string,
    ...
    const title = this.pickFirstString(record.title, record.name, record.goal, record.action, "Repository improvement");
    const rationale = this.pickFirstString(
      record.rationale,
      record.reason,
      record.why,
      record.description,
    ...
    const description = this.pickFirstString(record.description, record.summary, record.details, record.rationale, title);
    const dimensions = this.pickStringArray(
    ...
    const pathHint = this.pickFirstString(
      record.path,
      record.repoPath,
      (record.repo_context as { path?: unknown } | undefined)?.path,
      displayPath
    );
    const actionable = this.ensureActionableSuggestion(
      {
        title,
        description,
        rationale,
    ...
    const steps = this.pickStringArray(record.steps, record.actions, record.tasks, record.checklist);
  ...
  private buildFallbackSuggestPayload(
    targetPath: string,
    displayPath: string,
    context: string,
    maxSuggestions: number,
    ...
    const suggestions = this.synthesizeFallbackSuggestions(targetPath, context, Math.max(1, maxSuggestions), repoFiles)
      .map((suggestion) => ({
  ...
  private suggestOutputHasCandidates(rawOutput: unknown): boolean {
  ...
  private async generateSuggestOutput(
    suggestGoals: (
      context: string,
      options: {
        maxSuggestions: number;
    ...
    if (this.suggestOutputHasCandidates(firstAttempt)) {
      return firstAttempt;
    }
    ...
    return this.suggestOutputHasCandidates(retryAttempt) ? retryAttempt : firstAttempt;
  }
  ...
  private pickStringArray(...values: unknown[]): string[] {
    for (const value of values) {
  ...
  private async gatherProjectContext(_targetPath: string): Promise<string> {
    return "";
  }
  ...
  private async cmdSuggest(args: string[]): Promise<number> {
    ...
    try {
      suggestions = await this.generateSuggestOutput(
    ...
    const finalPayload = this.normalizeSuggestPayload(
      suggestions,
      targetPath,
      displayPath,
      fullContext,
