...
### Agent Execution
PulSeed delegates tasks to AI agents (e.g., Claude Code CLI, OpenAI Codex CLI) by spawning subprocesses. These agents can execute shell commands and file operations on the user's machine. **PulSeed does not provide OS-level sandboxing of agent processes.** The EthicsGate (L1) provides a software-level safety check — evaluating proposed actions against mechanical rules and, for ambiguous cases, an LLM judgment — but this is not a substitute for OS-level process isolation.
