...
# 7 curated goals aimed at improving Motiva itself.
# Keep descriptions concrete and verification-oriented so Motiva can drive changes.
GOALS=(
...
say "# Motiva overnight loop (${DATE_TAG})" > "$REPORT"
say "" >> "$REPORT"
...
for i in $(seq 1 7); do
  ...
  # 2) Run Motiva for that goal (bounded iterations)
  set +e
  ...
  say "## Motiva run" >> "$REPORT"
  say "- command: motiva run --goal ${GOAL_ID} --yes --max-iterations 12" >> "$REPORT"
  ...
  # 3) Verify repo health after Motiva-run-driven changes
  set +e
