...
# 7 curated goals aimed at improving Tavori itself.
# Keep descriptions concrete and verification-oriented so Tavori can drive changes.
GOALS=(
  "Fix 'tavori suggest' so it returns actionable improvement goals for this repo. Ensure it uses repo context (--path .) and doesn't prematurely return empty. Align behavior with docs/design where applicable. Add/adjust tests."
...
say "# Tavori overnight loop (${DATE_TAG})" > "$REPORT"
say "" >> "$REPORT"
...
for i in $(seq 1 7); do
  ...
  # 2) Run Tavori for that goal (bounded iterations)
  set +e
  ...
  say "## Tavori run" >> "$REPORT"
  say "- command: tavori run --goal ${GOAL_ID} --yes --max-iterations 12" >> "$REPORT"
  ...
  # 3) Verify repo health after Tavori-run-driven changes
  set +e
